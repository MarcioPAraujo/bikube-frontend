...
export const EmployeesFormSchema = yup.object().shape({
  ...
  funcao: yup.string().required('A função é obrigatória').min(3, 'A função deve ter pelo menos 3 caracteres'),
  data_nascimento: yup
    .string()
    .required('A data de nascimento é obrigatória')
    .matches(DDMMYYYY_REGEX, 'A data de nascimento deve estar no formato DD/MM/AAAA'),
  cpf: yup
    .string()
  ...
  dataentrada: yup
    ...
    .matches(DDMMYYYY_REGEX, 'A data de admissão deve estar no formato DD/MM/AAAA'),
  cep: yup
    .string()
