...
export const CredentialsSchema = yup.object().shape({
  email: yup.string().required('O email é obrigatório').email('Email inválido').matches(EMAIL_REGEX, 'Email inválido'),
  password: yup
    .string()
    ...
    .test('has-special-character', 'Deve conter ao menos um carater especial', value => {
      if (!value) return false;
