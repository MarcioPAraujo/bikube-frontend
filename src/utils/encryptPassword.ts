export const encryptPassword = async (password: string): Promise<string> => {
  ...
  const cryptoKey = await crypto.subtle.importKey('raw', keyData, { name: 'PBKDF2' }, false, [
    'deriveBits',
    'deriveKey',
  ]);
  ...
  const encryptedData = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, derivedKey, passwordData);
...
export const decryptPassword = async (encryptedHex: string): Promise<string> => {
  const key = process.env.NEXT_PUBLIC_ENCRYPT_KEY;
  if (!key) {
    throw new Error('Encryption key is not defined');
  }
  ...
  const decryptedData = await crypto.subtle.decrypt({ name: 'AES-GCM', iv }, derivedKey, data);
